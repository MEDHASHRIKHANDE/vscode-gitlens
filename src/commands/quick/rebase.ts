...
export class RebaseQuickCommand extends QuickCommandBase {
    constructor() {
        super('rebase', 'Rebase');
    }
            ...
            try {
                ...
                if (state.source === undefined || state.counter < 2) {
                    const step = this.createStep<BranchQuickPickItem>({
                        ...
                        placeholder: `Choose a branch to rebase ${state.destination.name} with`,
                        items: branches.map(
                            b =>
                ...
                const step = this.createConfirmStep(
                    ...
                    `${state.destination.name} with ${state.source.name}`
                );
                const selection = yield step;
